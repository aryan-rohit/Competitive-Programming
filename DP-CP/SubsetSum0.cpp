...
bool subSum(vector<int> &v ,int sum,int i,int count)
{
    if(i==v.size()){
        if(sum==0 && count!=0){  //count to check whether any element was taken or not as ques need non empty set.
            return true;
        }
        else{
            return false;
    ...
    bool take=subSum(v,sum+v[i],i+1,count+1);
    bool notTake=subSum(v,sum,i+1,count);
